...
class RatingController {
  async create(req, res, next) {
    try {
      ...
      const numberRate = Math.round(Number(rate));

      if (numberRate < 1 || numberRate > 5) {
        return next(ApiError.badRequest('Rating must be a whole number between 1 and 5'));
      }
      ...
      if (rating) {
        if (Number(rating.rate) === numberRate) {
          ...
          await Rating.update({ rate: numberRate }, { where: { deviceId, userId } });
        }
      } else {
        await Rating.create({ deviceId, userId, rate: numberRate });
      }
